...
- When running a cross-agent-tool review, do not wrap the invocation in a short timeout or treat early silence as failure. Run it as a long-running process and poll for progress. Judge unavailability from concrete process or provider evidence rather than a fixed waiting period. A failed provider preflight or one unavailable route does not block implementation, tests, CI repair, or other useful work: try the actual safe invocation or another independent reviewer, preserve completed work, and leave only the final independent-review gate blocked when no viable independent route remains.
